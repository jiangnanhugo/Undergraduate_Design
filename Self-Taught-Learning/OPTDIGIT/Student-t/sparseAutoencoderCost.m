...
rho=1/m*sum(log(a2.^2+1),2);
sparsity=sum(rho(:));
cost=cost+beta*sparsity;
sparsityGrad=1/m*sum( (2*a2)./(1+a2.^2) , 2);%(rho.^3+2*rho*epsilon).*(rho.^2+epsilon).^(-3/2);
%fprintf('sign equal ? %d\n',sparsityGrad==sign(rho));


delta3=-(data-a3).*sigmoidGradient(z3);
delta2=(W2'*delta3+beta*repmat(sparsityGrad,1,m)).*sigmoidGradient(z2);
